...
def get_comments(bvid, max_pages=5):
...
def main():
    ...
    latest_20 = without_episode[:20]
    
    print(f"找到 {len(latest_20)} 个最新发布的视频")
    print("=" * 50)
    
    for i, video in enumerate(latest_20):
        bvid = video['bvid']
        title = video['title'][:40]
        print(f"\n[{i+1}/20] 处理: {title}...")
        print(f"  BV号: {bvid}")
        
        comments = get_comments(bvid, max_pages=5)
